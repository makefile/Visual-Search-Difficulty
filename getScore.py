...
class CaffeExtractor:
    def __init__(self, gpu_id, model_def, model_weights, image_resize, mean_file):
        ...
        self.tr_features = np.asarray(joblib.load('feature/features-pyramid-vgg-f.pkl')).T
        self.svr = joblib.load('svr.pkl')

    def extractFeature(self, image_file_list, image_base_path, batch_size=1):
    ...
    def predict_score(self, image_file_list, image_base_path, batch_size=1):
        featureVectors = self.extractFeature(image_file_list, image_base_path, batch_size)
        Kx = np.asarray(featureVectors).dot(self.tr_features)
        testScores = self.svr.predict(Kx)
        return testScores
...
if __name__ == "__main__":
    ...
    ims = ['2008_000002','2008_000015','2008_000016','2008_000237','2008_001613','2011_003253']
    image_file_list = [image_base_path + x + '.jpg' for x in ims]
    extractor = CaffeExtractor(gpu_id, model_def, model_weights, image_resize, mean_file)
    testScores = extractor.predict_score(image_file_list, image_base_path, batch_size=1)
    print(testScores)
